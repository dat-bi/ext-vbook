function execute(url) {
    if(url.indexOf("modules/obook/reader") !== -1){
        ...
        if (response.ok) {
            ...
            var i1 = text.replace(/<i class=\"webfont\">/g,"").replace(/<\/i>/g,"").replace(/\n|\s/g,"").replace(/&(nbsp|amp|quot|lt|gt|bp|emsp);/g, "").replace(/<!doctypehtml>(.*?)<divclass="acontent"id="ccontent">/g,"").replace(/<spanstyle=.*?>.*?<\/span>/g,"").replace(/<\/div>.*?<\/html>/g,"")

            if(text.includes("blockcontent")){
